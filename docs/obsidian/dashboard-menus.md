...
> 내부 링크 예: `[[dashboard-menus#업무]]` 처럼 앵커를 붙여 쓸 수 있습니다.
...
- **플랫폼 관리자 + 단일 테넌트 모드 아님**  
  - 그룹 **플랫폼**: **거래처 선택** (`/dashboard/select-tenant`), **감사 로그** (`/dashboard/audit`)
- **그 외**  
...
소스: `src/lib/dashboard-nav.ts` — 그룹 **2개**: **업무 · 관리**  
실무 화면은 업무 순서대로 한 그룹에 나열한다(시작/업무 이중 구분 없음).

### 업무

| 메뉴 | 경로 | 주요 내용 |
|------|------|-----------|
| 대시보드 | `/dashboard` | 요약·지표·적립 패널·**바로 가기**(직원·규칙·스케줄·보고·신고), 설정 안내 |
...
| 운영 보고 | `/dashboard/operating-report` | 운영보고서 탭들(미리보기, 기본정보, 기본재산, 기금운용, 기금재원, 사용현황, 사업실적, 부동산) |
| 급여 포함 신고 | `/dashboard/salary-inclusion-report` | 급여 포함 신고 관련 화면 |
...
| 탭(표시명) | 설명 |
|----|------|
| 정기 지급 | 연도·레벨별 정기 지급 금액 규칙 |
| 분기 요율 | 분기 항목별 요율(영유아 등) 매트릭스 |
| 분기 체크 | 분기 지원 대상 일괄 체크 그리드 |
| 분기 개별 | 직원·항목별 분기 설정 폼 |
...
| 탭(표시명) | 설명 |
|----|------|
| 스케줄 | 직원별 12개월 지급 합계·내역, 지급완료 표시, **월별 개별 금액 수정 모달**(정기 일정 없이도 **월 임의 지급** `MONTHLY_ADHOC` 키로 금액만 넣기 가능, 그 외 항목은 「＋ 항목 추가」) |
| 안내 | 카카오·문자 등 안내 문구 생성 |
| 메모·인센 | 월별 발생 인센 그리드 + 선택적 복지·메모 폼 |
| 적립금 | 자본금 50%·추가 적립 진행 노트 등 |
| 예정액 | 레벨·연간 예정 사복 등 배정 UI |
| 연봉 점검 | 조사표 `adjustedSalary` vs 월별 누적 진단, **재동기화**(개별·일괄) |
